...
proxyMT.__index = function(t, k)
    ...
    -- path compatibility (renderer reads/writes path.width/height for resize)
    if k == "path" then
        local cached = rawget(t, "_pathProxy")
        if cached then return cached end
        local proxy = t  -- capture reference
        local pathProxy = setmetatable({}, {
            __index = function(_, pk)
                local p = rawget(proxy, "_params")
                if pk == "width" then return p.width
                elseif pk == "height" then return p.height end
            end,
            __newindex = function(_, pk, pv)
                local p = rawget(proxy, "_params")
                if pk == "width" then p.width = pv
                elseif pk == "height" then p.height = pv end
                -- Note: SDF shapes can't dynamically resize the shader rect.
                -- The renderer will recreate the object when borderRadius > 0.
            end,
        })
        rawset(t, "_pathProxy", pathProxy)
        return pathProxy
    end

    -- fill compatibility (renderer reads fill.r/g/b/a to preserve color on recreate)
    if k == "fill" then
        local fc = rawget(t, "_fillColor")
        if fc then
            return { r = fc[1] or 0, g = fc[2] or 0, b = fc[3] or 0, a = fc[4] or 1 }
        end
